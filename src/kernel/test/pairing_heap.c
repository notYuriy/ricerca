...
static bool pairing_heap_compare(struct pairing_heap_node *left, struct pairing_heap_node *right) {
...
void test_pairing_heap(void) {
	...
	for (int ii = 128; ii > 0; ii -= 2) {
		int i = ii - 2;
		nodes[i].key = i;
		pairing_heap_insert(&heap, (struct pairing_heap_node *)(nodes + i));
	}
